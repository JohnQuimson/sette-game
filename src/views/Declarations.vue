...
const declarationValue = ref(null); // Valore inserito nel popup per il giocatore corrente
...
const currentPlayer = computed(
  () => declarationOrder.value[activePlayerIndex.value],
);

// Controlla se il giocatore corrente è il dealer (sarà l'ultimo a dichiarare)
const isCurrentPlayerDealer = computed(
  () => currentPlayer.value?.id === game.currentDealerId,
);

// Errore se il dealer inserisce la dichiarazione vietata
const isDealerInvalid = computed(() => {
  return (
    isCurrentPlayerDealer.value &&
    declarationValue.value === game.forbiddenDealerDeclaration
  );
});
...
onMounted(() => {
  if (currentPlayer.value) {
    declarationValue.value =
      currentPlayer.value.declared !== null &&
      currentPlayer.value.declared !== undefined
...
function nextDeclaration() {
  ...
  const playerInStore = game.players.find(
  ...
  if (activePlayerIndex.value < declarationOrder.value.length - 1) {
    ...
    declarationValue.value =
      nextPlayer.declared !== null && nextPlayer.declared !== undefined
        ? nextPlayer.declared
        : 0;
  } else {
        ...
        <div class="modal-content shadow-lg border-primary">
          ...
          <div class="modal-content py-4 px-4 text-center">
            ...
            <h3 class="fw-bold border-bottom pb-2 text-uppercase text-primary">
              {{ currentPlayer?.name }}
              <span
                v-if="isCurrentPlayerDealer"
                class="fs-6 text-danger d-block mt-1"
                >(È il Dealer)</span
              >
            </h3>
            ...
            >
              ❌ Non puoi dichiarare
              <strong>{{ game.forbiddenDealerDeclaration }}</strong>
            </div>

            <div class="declaration-area my-4 mx-auto">
              <div class="input-box">
                ...
                <div v-if="isDealerInvalid" class="invalid-feedback fw-bold">
                  Dichiarazione vietata!
                </div>
              </div>
            ...
            <div class="d-flex gap-2">
              ...
              <button
                ...
                :disabled="
                  ...
                  declarationValue < 0 ||
                  declarationValue > game.currentCards
                "
                @click="nextDeclaration"
              >
                {{
                  activePlayerIndex === declarationOrder.length - 1
                    ? 'Termina e Gioca'
                    : 'Avanti >'
                }}
              </button>
            ...
            <small class="text-muted d-block mt-3">
              Giocatore {{ activePlayerIndex + 1 }} di
              {{ declarationOrder.length }}
            </small>
          </div>
        </div>
      </div>
